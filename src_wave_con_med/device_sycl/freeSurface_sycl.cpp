...
static sycl::range local{1, 4, 32}; // original
	...
	int _nx_, int _ny_, int _nz_, float rDH, int FB1, int FB2, int FB3, float DT, sycl::queue &Q)
{
    ...
    sycl::range global{HALO/local[0]*local[0], 
    ...
    Q.parallel_for(sycl::nd_range{global, local}, [=](sycl::nd_item<3> nd_item){
        ...
        int k = nd_item.get_global_id(0) + _nz - HALO;
        ...
        CALCULATE3D_SYCL( i, j, k, HALO, _nx, HALO, _ny, _nz - HALO, _nz )		
            index = INDEX( i, j, k ); 								
